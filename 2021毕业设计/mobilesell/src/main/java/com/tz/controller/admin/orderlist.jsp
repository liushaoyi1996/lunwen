...
<html>
 ...
 <div style="margin-top:0px;margin-left:15px;padding-right:0px;height:535px;background-color:#ffffff" class="col-xs-9 col-md-9">
  ...
  <c:forEach items="${olist }" var="o">
    <tr class="danger">
      ...
      <td>${o.num }</td>
      <td>${o.price }</td>
        <c:if test="${o.status==1 }"><td>审核通过</td></c:if>
        <c:if test="${o.status==2 }"><td>审核不通过</td></c:if>
        <c:if test="${o.status==0 }"><td>待审核</td></c:if>
        <td><button class="btn btn-info" onclick="shenhe(${o.id})">审核通过</button><button class="btn btn-info" onclick="shenhea(${o.id})">审核不通过</button></td>
     </tr>
   </c:forEach>
  </tbody>
</table>
 ...
 <script>
   ...
   function shenhea(id){
       if(confirm("确定审核不通过吗？")){
           window.location.href="${path}/admin/shenhea?id="+id;
       }
   }
 </script>
</html>
